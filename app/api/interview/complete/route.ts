...
export async function POST(req: NextRequest) {
  try {
    ...
    if (userId) {
      const isRecruiter = existingCandidate.userId === userId;
      const isCandidate =
        existingCandidate.clerkUserId === userId || // Primary: clerkUserId match
        (candidateEmail && existingCandidate.email === candidateEmail); // Fallback: email match
