...
async function fix() {
  ...
  await run('Create Candidates', 'CREATE TABLE candidates (id serial PRIMARY KEY, user_id varchar(255) NOT NULL, target_job_id integer REFERENCES jobs(id) ON DELETE SET NULL, job_title text, name text NOT NULL, email text NOT NULL, phone text NOT NULL, resume_text text, status varchar(50) DEFAULT \'Ready\' NOT NULL, score text, match_score text, transcript text, summary text, created_at timestamp DEFAULT now() NOT NULL)');
}
