        ...
        <div className="p-8 space-y-6">
          <DialogHeader>
            <div className="flex items-center gap-3 mb-2">
              ...
              <DialogTitle className="text-2xl font-bold tracking-tight text-slate-900">Post a New Opportunity</DialogTitle>
            </div>
            <p className="text-slate-500 text-sm">Fill in the details below or let our AI help you draft the perfect role.</p>
          </DialogHeader>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <Label htmlFor="title" className="text-xs font-bold text-slate-600 uppercase tracking-wider">Job Title *</Label>
                <Input
                  id="title"
                ...
                />
                {errors.title && (
                  <p className="text-[10px] font-bold text-rose-600 uppercase tracking-wider mt-1">{errors.title.message}</p>
                )}
              </div>
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    {...register("location")}
                    className={`bg-slate-50 border-slate-200 text-slate-900 h-12 rounded-xl pl-10 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 ${
                      errors.location ? "border-rose-400 focus:ring-rose-500/20" : ""
                    }`}
                  />
                </div>
                {errors.location && (
                  <p className="text-[10px] font-bold text-rose-600 uppercase tracking-wider mt-1">{errors.location.message}</p>
                )}
              </div>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <Label htmlFor="description" className="text-xs font-bold text-slate-600 uppercase tracking-wider">Job Description *</Label>
                <Button 
                  type="button" 
                  variant="ghost" 
                  size="sm" 
                  ...
                  className="h-7 text-[10px] font-bold text-indigo-600 hover:text-indigo-800 hover:bg-indigo-50 rounded-lg gap-1.5"
                >
                  {generating ? <Loader2 className="w-3 h-3 animate-spin" /> : <Sparkles className="w-3 h-3 text-indigo-600" />}
                  Generate with AI
                </Button>
              </div>
              <Textarea
                ...
                className={`bg-slate-50 border-slate-200 text-slate-900 min-h-[150px] rounded-2xl p-4 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 resize-none ${
                  errors.description ? "border-rose-400 focus:ring-rose-500/20" : ""
                }`}
              />
              {errors.description && (
                <p className="text-[10px] font-bold text-rose-600 uppercase tracking-wider mt-1">{errors.description.message}</p>
              )}
            </div>

            <div className="space-y-2">
              <Label htmlFor="requirements" className="text-xs font-bold text-slate-600 uppercase tracking-wider">Requirements (Optional)</Label>
              <Input
                id="requirements"
                placeholder="e.g. 5+ years React, TypeScript, Node.js"
                {...register("requirements")}
                className={`bg-slate-50 border-slate-200 text-slate-900 h-12 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 ${
                  errors.requirements ? "border-rose-400 focus:ring-rose-500/20" : ""
                }`}
              />
              {errors.requirements && (
            ...
            <div className="pt-4 flex gap-4">
              ...
              <Button type="submit" disabled={loading} className="flex-1 h-12 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold shadow-md shadow-indigo-500/20">
